...
#if defined(WIN32)
  char test_udsdir[] = "envoy_test_uds.XXXXXX";
  int size = strlen(test_udsdir) + 1; // terminating NULL
  RELEASE_ASSERT(_mktemp_s(test_udsdir, size) == 0);

  std::error_code ec;
  auto tmp_dir = fs::temp_directory_path(ec);
  RELEASE_ASSERT(!ec);

  fs::path ret_path = tmp_dir;
  ret_path /= test_udsdir;
  fs::create_directories(ret_path, ec);
  RELEASE_ASSERT(!ec);
...
#if defined(WIN32)
  fs::path out_dir = out_json_path;
  std::error_code ec;
  fs::create_directories(out_dir.parent_path(), ec);
  RELEASE_ASSERT(!ec);
#else
...
#if defined(WIN32)
  //TODO: Is setting PYTHONPATH here required on windows?  
  //fs::path out_dir = args[0];
  //cmd << "set PYTHONPATH=" << out_dir.parent_path() << " && ";
#else
